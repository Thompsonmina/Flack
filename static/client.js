...
document.addEventListener("DOMContentLoaded", () => {
	// set the profile name, compile template then insert into dom
	const template = Handlebars.compile(document.querySelector("#usertemplate").innerHTML);
	const user = template({"name": username});
	document.querySelector(".userprofile").innerHTML = user;
	...
	document.querySelectorAll(".publicButtn").forEach(btn => {
	...
	socket.on("connect", () => {
		...
		document.querySelectorAll('#create-publicbtn').forEach(btn => {
	    	
	    	btn.onclick = () => {
		    	const channel = prompt(`Create Public Channel \nEnter the channel name:`);
		    	
		    	// ensures that the channel exist and it is unique and it doesnt pass a set lenght
		    	if (channel && channelOccurence(channel, "publicButtn") == 0 && channel.length <= 15) 
		    	{
		    		console.log("emmited")
	...
	socket.on("show newchannel", data => {
		...
		buttn.className = "buttn publicButtn";
		buttn.name =  data.channel;
		buttn.innerHTML = data.channel;
		buttn.setAttribute("data-type", "public")
		...
		document.querySelector(".channelbuttons").append(buttn);
	});
